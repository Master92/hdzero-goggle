...
static const char * const heading = "Playback:";
...
static lv_obj_t *page_playback_create(lv_obj_t *parent, panel_arr_t *arr) {
    ...
    create_text(&textTmp, section, false, heading, LV_MENU_ITEM_BUILDER_VARIANT_2);
...
static void update_page() {
    ...
    sprintf(tmp_string, "%s %s", heading, &currentFolder->filename[strlen(REC_diskPATH)]);
